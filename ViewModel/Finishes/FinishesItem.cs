...
{
    public class FinishesItem: INotifyPropertyChanged
    {
		private int _itemNo;
		...
		private ObservableCollection<FinishesBreakdownLine> _finishesBreakdownLines =
			new ObservableCollection<FinishesBreakdownLine>();
		public ObservableCollection<FinishesBreakdownLine> FinishesBreakdownLine
		{
			get => _finishesBreakdownLines;
			set
			{
				if (_finishesBreakdownLines != value)
				{
					_finishesBreakdownLines = value;
					OnPropertyChanged(nameof(FinishesBreakdownLine));
				}
			}
		}
