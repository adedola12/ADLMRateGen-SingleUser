...
namespace ADLMRateGen.View
{
    public partial class PaintworkDetailControl : UserControl
    {
        ...
        private void QuantityTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (tb.DataContext is not PaintingBreakdownLine line) return;
            if (DataContext is not PaintWorkItem item) return;
        ...
        private async void ResetThisRateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            await RateEditCommands.ResetItemAsync(SectionKeys.Paint, item.ItemNo);
        }
        ...
        private static bool TryGetParentVm(out PaintWorkViewModel? vm)
        {
            vm = null;
            if (Application.Current?.MainWindow?.DataContext is MainViewModel main)
            {
                vm = main.PaintWorkViewModel;
                return vm != null;
            }
            return false;
        }
