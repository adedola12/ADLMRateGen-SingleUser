...
{
    /// <summary>
    /// Interaction logic for WindowAndDoorView.xaml
    /// </summary>
    public partial class WindowAndDoorView : UserControl
    {
        public WindowAndDoorView()
        {
            InitializeComponent();
		...
		{
			if (e.PropertyName != nameof(WindowAndDoorViewModel.SelectedDetail))
				return;

			Dispatcher.Invoke(() =>
			{
				var vm = (WindowAndDoorViewModel)sender;

				if (vm.SelectedDetail is UserControl detailView)
					GlobalPopup.Show(detailView);
				else
					GlobalPopup.Hide();
			});
		}
