...
namespace ADLMRateGen.View
{
    public partial class GroundworkItemDetailControl : UserControl
    {
        ...
        private void QuantityTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (tb.DataContext is not GroundworkBreakdownLine line) return;
            if (DataContext is not GroundworkItem item) return;
        ...
        private async void ResetThisRateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            await RateEditCommands.ResetItemAsync(SectionKeys.Ground, item.ItemNo);
        }
        ...
        private static bool TryGetParentVm(out GroundWorkViewModel? vm)
        {
            vm = null;
            if (Application.Current?.MainWindow?.DataContext is MainViewModel main)
            {
                vm = main.GroundWorkViewModel;
                return vm != null;
            }
            return false;
        }
