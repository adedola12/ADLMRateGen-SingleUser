...
namespace ADLMRateGen.View
{
    public partial class FinishesDetailControl : UserControl
    {
        ...
        private void QuantityTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (tb.DataContext is not FinishesBreakdownLine line) return;
            if (DataContext is not FinishesItem item) return;
            if (string.IsNullOrWhiteSpace(line.ComponentName)) return;
            if (line.IsTotalLine) return;

            UserRateEditStore.Current.SetOverride(
                SectionKeys.Finishes,
                item.ItemNo,
                line.ComponentName,
                line.Quantity);
        ...
        private async void ResetThisRateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            await RateEditCommands.ResetItemAsync(SectionKeys.Finishes, item.ItemNo);
        }
        ...
        private static bool TryGetParentVm(out FinishesViewModel? vm)
        {
            vm = null;
            if (Application.Current?.MainWindow?.DataContext is MainViewModel main)
            {
                vm = main.FinishesViewModel;
                return vm != null;
            }
            return false;
        }
